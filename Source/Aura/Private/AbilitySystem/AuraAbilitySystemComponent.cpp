...
void UAuraAbilitySystemComponent::ForEachAbility(const FForEachDelegate& Delegate)
{
	FScopedAbilityListLock ActiveScopeLock(*this);
	...
	const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			// SlotIsEmpty(Slot)
			if (!SlotIsEmpty(Slot))		// There is an ability in the slot already. Deactivate and clear its slots 
			{
				FGameplayAbilitySpec* SpecWithSlot = GetSpecWithSlot(Slot);
				if (SpecWithSlot)
				{
					// is that ability the same as this ability? If so, we can do early
			...
			if (!AbilityHasAnySlot(*AbilitySpec))   // ability doesnt have a slot / its not active
			{
...
bool UAuraAbilitySystemComponent::GetDescriptionsByAbilityTags(const FGameplayTag& AbilityTag, FString& OutDescription,
